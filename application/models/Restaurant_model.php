...
class Restaurant_model extends CI_Model
{
	...
	{
		if($user_id){
			$this->db->where("restaurant_id NOT IN (SELECT restaurant_id FROM tbl_restaurant_owners where fk_user_id!=$user_id)", NULL, FALSE);
		}else{

		$this->db->where('`restaurant_id` NOT IN (SELECT `restaurant_id` FROM `tbl_restaurant_owners`)', NULL, FALSE);
		}
		$this->db->where('is_active','1');
		$query=$this->db->get('tbl_restaurants');
		return $query->result();
